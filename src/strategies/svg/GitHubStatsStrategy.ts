...
const ICONES = {
  ...
  REPOSITORIOS: `<path d="M4 3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2H4zm0 2h16v14H4V5zm2 2v2h2V7H6zm4 0v2h8V7h-8zm-4 4v2h2v-2H6zm4 0v2h8v-2h-8zm-4 4v2h2v-2H6zm4 0v2h8v-2h-8z" fill="currentColor"/>`,
...
export class GitHubStatsStrategy implements SVGStrategy {
  ...
  generate(DADOS: unknown, username: string, config?: CardConfig): string {
    ...
    return `<svg width="${config?.LARGURA ?? LARGURA}" height="${config?.ALTURA ?? ALTURA}" viewBox="0 0 ${LARGURA} ${ALTURA}" xmlns="http://www.w3.org/2000/svg">
<defs>
  <linearGradient id="bgGrad" x1="0%" y1="0%" x2="100%" y2="100%">
    <stop offset="0%" style="stop-color:${CORES.bgGradient[0]};stop-opacity:1" />
    <stop offset="100%" style="stop-color:${CORES.bgGradient[1]};stop-opacity:1" />
  </linearGradient>
  <linearGradient id="accentGrad" x1="0%" y1="0%" x2="100%" y2="0%">
    <stop offset="0%" style="stop-color:${CORES.accentGradient[0]};stop-opacity:1" />
    <stop offset="100%" style="stop-color:${CORES.accentGradient[1]};stop-opacity:1" />
  </linearGradient>
  <linearGradient id="shimmer" x1="0%" y1="0%" x2="100%" y2="0%">
    <stop offset="0%" style="stop-color:${CORES.primaryColor};stop-opacity:0.3">
      <animate attributeName="offset" values="0;1;0" dur="3s" repeatCount="indefinite"/>
    </stop>
    <stop offset="50%" style="stop-color:${CORES.secondaryColor};stop-opacity:0.5">
      <animate attributeName="offset" values="0.5;1;0.5" dur="3s" repeatCount="indefinite"/>
    </stop>
    <stop offset="100%" style="stop-color:${CORES.primaryColor};stop-opacity:0.3">
...
<rect width="${LARGURA}" height="${ALTURA}" rx="${RAIOS_BORDA}" fill="url(#bgGrad)"/>

${MOSTRAR_BORDA ? `<rect x="${LARGURA_BORDA / 2}" y="${LARGURA_BORDA / 2}" width="${LARGURA - LARGURA_BORDA}" height="${ALTURA - LARGURA_BORDA}" rx="${RAIOS_BORDA - 1}" fill="none" stroke="${CORES.borderColor}" stroke-width="${LARGURA_BORDA}" filter="url(#glow)"/>` : ""}

<rect x="0" y="0" width="${LARGURA}" height="4" fill="url(#shimmer)" rx="${RAIOS_BORDA}" />

<rect x="${PADDING}" y="${PADDING}" width="${LARGURA - PADDING * 2}" height="60" rx="8" fill="${CORES.cardBg}" opacity="0.6"/>
<rect x="${PADDING}" y="${PADDING}" width="6" height="60" rx="3" fill="url(#accentGrad)"/>
<text x="${PADDING + 20}" y="${PADDING + 28}" font-family="'Segoe UI', Ubuntu, Arial, sans-serif" font-size="22" font-weight="700" fill="${CORES.primaryColor}">${escapeXml(username)}</text>
<text x="${PADDING + 20}" y="${PADDING + 50}" font-family="'Segoe UI', Ubuntu, Arial, sans-serif" font-size="13" fill="${CORES.COR_TEXTO}" opacity="0.7">GitHub Statistics</text>
<circle cx="${LARGURA - PADDING - 25}" cy="${PADDING + 30}" r="6" fill="${CORES.accentGradient[0]}">
...
${DADOS_ESTATISTICAS.map((stat, statIndex) => {
  ...
  return `<g>
    <rect x="${POSICAO_X}" y="${POSICAO_Y}" width="${LARGURA_CARTAO}" height="${ALTURA_CARTAO}" rx="10" fill="${CORES.cardBg}" stroke="${CORES.borderColor}" stroke-width="1.5"/>
    <svg x="${POSICAO_X + PADDING_CARTAO}" y="${POSICAO_Y + PADDING_CARTAO}" width="24" height="24" viewBox="0 0 24 24">
      <g style="color: ${stat.color}">${ICONES[stat.icon as keyof typeof ICONES]}</g>
    </svg>
    <text x="${POSICAO_X + PADDING_CARTAO}" y="${POSICAO_Y + ALTURA_CARTAO - 30}" font-family="'Segoe UI', Ubuntu, Arial, sans-serif" font-size="28" font-weight="700" fill="${stat.color}">${stat.VALOR}</text>
    <text x="${POSICAO_X + PADDING_CARTAO}" y="${POSICAO_Y + ALTURA_CARTAO - 12}" font-family="'Segoe UI', Ubuntu, Arial, sans-serif" font-size="12" fill="${CORES.COR_TEXTO}" opacity="0.7">${stat.label}</text>
    <rect x="${POSICAO_X}" y="${POSICAO_Y}" width="4" height="${ALTURA_CARTAO}" rx="10" fill="${stat.color}" opacity="0.6"/>
  </g>`;
}).join("")}

<text x="${LARGURA / 2}" y="${ALTURA - 12}" text-anchor="middle" font-family="'Segoe UI', Ubuntu, Arial, sans-serif" font-size="10" fill="${CORES.COR_TEXTO}" opacity="0.4">Galeria I.C.L • github-stats</text>
</svg>`;
  }
