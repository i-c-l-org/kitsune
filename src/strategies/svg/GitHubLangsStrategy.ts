...
function renderBars(
  ...
  return LINGUAS.map((lang, langIndex) => {
    ...
    return `<g>
        <rect x="${INICIO_X}" y="${POSICAO_Y}" width="${LARGURA_AREA_BAR}" height="${ALTURA_BAR}" rx="8" fill="${CORES.cardBg}" />
        <rect x="${INICIO_X}" y="${POSICAO_Y}" width="${LARGURA_BAR}" height="${ALTURA_BAR}" rx="8" fill="${lang.color}" />
        <text x="${INICIO_X + 12}" y="${POSICAO_Y + 18}" fill="${CORES.COR_TEXTO}" font-size="13" font-weight="600">${lang.NOME}</text>
        <text x="${LARGURA - 120}" y="${POSICAO_Y + 18}" fill="${CORES.COR_TEXTO}" font-size="13" text-anchor="end" font-weight="700">${formatPercent(lang.PERCENTUAL)}</text>
      </g>`;
  }).join("");
}
...
<rect width="${LARGURA}" height="${ALTURA}" rx="${RAIOS_BORDA}" fill="url(#bgLangGrad)" />
...
<text x="28" y="70" fill="${CORES.COR_TEXTO}" font-size="14">${USUARIO_SEGURO} · ${ROTULO_TOTAL}</text>

${renderBars(LINGUAS, CORES, LARGURA)}

<text x="${LARGURA - 20}" y="${ALTURA - 18}" fill="${CORES.COR_TEXTO}" font-size="11" text-anchor="end" opacity="0.7">Atualização em tempo real</text>
</svg>`;
  }
